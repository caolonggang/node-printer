var printer_helper = {},
    ...
    os = require("os"),
    path = require("path"),
    binding_path = path.resolve(__dirname, '../builds/node_printer_'+process.platform+'_'+process.arch+'.node'),
    printer_helper;

if(fs.existsSync(binding_path)) {
    printer_helper = require(binding_path);
} else {
	throw new Error('The pre build lib for the platform' + process.platform + 'does not exist.');
}
